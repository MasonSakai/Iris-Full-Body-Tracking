...
import { tag_list, camera_list, TagInfo, CameraInfo, selected, Refresh } from './network'
import { Transform } from 'node:stream'
import { camera, controls, LookAt, scene } from './apriltag3D'



export function UpdateFrom(srcIdent: string) {

	var ignore: string[] = []
	var queue: string[] = [srcIdent]
	...
	while (queue.length > 0) {
		...
		if (isNaN(+srcIdent)) {
			...
			for (const id in tag.cams) {
				...
				if (ignore.includes(cam.ident.toString()) || queue.includes(cam.ident.toString())) continue
			...
			for (const ident in tag_list) {
				...
				if (!(cam.ident in tag.cams)) continue
				if (ignore.includes(tag.ident) || queue.includes(tag.ident)) continue
