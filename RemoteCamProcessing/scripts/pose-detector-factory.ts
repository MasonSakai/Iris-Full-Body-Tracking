...
export function setBackend(backend: "webgl" | "webgpu" = "webgpu") {
	return tf.setBackend("webgpu");
}

export function tfReady() { return tf.ready() }

export async function CreateDetector(model_type = "SINGLEPOSE_THUNDER"): Promise<posedetection.PoseDetector> {
	await tf.ready();
...
export async function GetFilteredPose(image, detector: posedetection.PoseDetector, threshold: number = 0.3, flipHorizontal: boolean = false, maxPoses: number = 1): Promise<{ [key: string]: { x: number, y: number, score: number } }[]> {
