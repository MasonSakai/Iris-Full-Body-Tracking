...
def TagIdent(det: Detection):
    return (det.tag_family, det.tag_id)
...
def ScanTags(num_scans: int = 1, scan_sep: float = 0.25, ref_list: list[CameraReference] = []):
    ...
    ref_dets: dict[tuple[str, int], dict[Camera, tuple[float, int, np.ndarray, np.ndarray, float, float]]] = {}
    ref_tags: dict[AprilTag, dict[Camera, tuple[int, np.ndarray, np.ndarray, float, float]]] = {}
    ...
    for cam in cam_list:
        ...
        for ident, dets in all_dets.items():
            ...
            if ident not in ref_dets:
                ref_dets[ident] = {}
            ref_dets[ident][cam] = (base_size, len(dets), pos, rot, v_pos, v_mar)
