...
public:
    ...
    void validate()
    {
        ...
        else
        {
            ...
            if (inputType == CAMERA)
                inputCapture = new VideoCapture(cameraID);
            if (inputType == VIDEO_FILE)
                inputCapture = new VideoCapture(input);
...
public:
    ...
    int arucoDictID;             // ID of preexisting dictionary
...
static inline void read(const FileNode& node, Settings2& x, const Settings2& default_value = Settings2())
{
    if (node.empty())
        x = default_value;
    else
...
bool runCalibrationAndSave(Settings2& s, Size imageSize, Mat& cameraMatrix, Mat& distCoeffs,
...
int main(int argc, char* argv[])
{
    ...
    if (s.calibrationPattern == Settings2::Pattern::CHARUCOBOARD) {
    ...
    if (s.calibrationPattern == Settings2::CHARUCOBOARD) {
        dictionary = aruco::getPredefinedDictionary(s.arucoDictID);
    }
    else {
        // default dictionary
    ...
    Size imageSize;
    int mode = s.inputType == Settings2::IMAGE_LIST ? CAPTURING : DETECTION;
    ...
    for (;;)
    {
        ...
        if (found)                // If done with success,
        {
            ...
            if (s.calibrationPattern == Settings2::CHESSBOARD)
            {
                Mat viewGray;
    ...
    //! [show_results]
    if (s.inputType == Settings2::IMAGE_LIST && s.showUndistorted && !cameraMatrix.empty())
    {
...
static void calcBoardCornerPositions(Size boardSize, float squareSize, vector<Point3f>& corners,
    Settings2::Pattern patternType /*= Settings2::CHESSBOARD*/)
{
    corners.clear();
...
//! [board_corners]
static bool runCalibration(Settings2& s, Size& imageSize, Mat& cameraMatrix, Mat& distCoeffs,
...
static void saveCameraParams(Settings2& s, Size& imageSize, Mat& cameraMatrix, Mat& distCoeffs,
    const vector<Mat>& rvecs, const vector<Mat>& tvecs,
